...
public class FieldSerializer implements Comparable<FieldSerializer> {
        ...
        if (annotation != null) {
            ...
            while (true) {
                if (r4 >= length) {
                    ...
                    if ((serializerFeatureArrSerialzeFeatures[r4].getMask() & SerializerFeature.WRITE_MAP_NULL_FEATURES) != 0) {
                        z = true;
                        break;
                    }
                    r4++;
            ...
            this.features = SerializerFeature.m22of(annotation.serialzeFeatures()) | this.features;
        } else {
            z = false;
        }
        this.writeNull = z;
    ...
    public void writeValue(JSONSerializer jSONSerializer, Object obj) throws Exception {
        ...
        int r8 = (this.disableCircularReferenceDetect ? this.fieldInfo.serialzeFeatures | SerializerFeature.DisableCircularReferenceDetect.mask : this.fieldInfo.serialzeFeatures) | this.features;
        if (obj == null) {
            SerializeWriter serializeWriter = jSONSerializer.out;
            ...
            if (serializeWriter.isEnabled(SerializerFeature.WRITE_MAP_NULL_FEATURES) && (objectSerializer instanceof JavaBeanSerializer)) {
                ...
                objectSerializer.write(jSONSerializer, null, this.fieldInfo.name, this.fieldInfo.fieldType, r8);
                return;
            }
        }
        ...
        if (this.fieldInfo.unwrapped) {
            if (objectSerializer2 instanceof JavaBeanSerializer) {
                ((JavaBeanSerializer) objectSerializer2).write(jSONSerializer, obj, this.fieldInfo.name, this.fieldInfo.fieldType, r8, true);
                return;
            } else if (objectSerializer2 instanceof MapSerializer) {
                ((MapSerializer) objectSerializer2).write(jSONSerializer, obj, this.fieldInfo.name, this.fieldInfo.fieldType, r8, true);
                return;
            }
        }
        if ((this.features & SerializerFeature.WriteClassName.mask) != 0 && cls3 != this.fieldInfo.fieldClass && (objectSerializer2 instanceof JavaBeanSerializer)) {
            ((JavaBeanSerializer) objectSerializer2).write(jSONSerializer, obj, this.fieldInfo.name, this.fieldInfo.fieldType, r8, false);
            return;
        }
        ...
        objectSerializer2.write(jSONSerializer, obj, this.fieldInfo.name, this.fieldInfo.fieldType, r8);
    }
