...
public class LoadPath<Data, ResourceType, Transcode> {
    ...
    public Resource<Transcode> load(DataRewinder<Data> dataRewinder, Options options, int r11, int r12, DecodePath.DecodeCallback<ResourceType> decodeCallback) throws GlideException {
        List<Throwable> list = (List) Preconditions.checkNotNull(this.listPool.acquire());
        try {
            return loadWithExceptionList(dataRewinder, options, r11, r12, decodeCallback, list);
        } finally {
            this.listPool.release(list);
        }
    }

    private Resource<Transcode> loadWithExceptionList(DataRewinder<Data> dataRewinder, Options options, int r15, int r16, DecodePath.DecodeCallback<ResourceType> decodeCallback, List<Throwable> list) throws GlideException {
        int size = this.decodePaths.size();
        Resource<Transcode> resourceDecode = null;
        for (int r5 = 0; r5 < size; r5++) {
            try {
                resourceDecode = this.decodePaths.get(r5).decode(dataRewinder, r15, r16, options, decodeCallback);
