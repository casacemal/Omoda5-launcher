...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ...
                next.executor.execute(new CallResourceReady(next.f92cb));
            }
    ...
    synchronized void incrementPendingCallbacks(int r3) {
        ...
        if (this.pendingCallbacks.getAndAdd(r3) == 0 && this.engineResource != null) {
    ...
    void decrementPendingCallbacks() {
        ...
        synchronized (this) {
            this.stateVerifier.throwIfRecycled();
            Preconditions.checkArgument(isDone(), "Not yet complete!");
            int r0 = this.pendingCallbacks.decrementAndGet();
            Preconditions.checkArgument(r0 >= 0, "Can't decrement below 0");
            if (r0 == 0) {
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ResourceCallbackAndExecutor next = it.next();
                next.executor.execute(new CallLoadFailed(next.f92cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    private class CallLoadFailed implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f90cb;
        ...
        public void run() {
            synchronized (this.f90cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f90cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f90cb);
                    }
    ...
    private class CallResourceReady implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f91cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f91cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f91cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f91cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f91cb);
                        EngineJob.this.removeCallback(this.f91cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        /* renamed from: cb */
        final ResourceCallback f92cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f92cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f92cb.equals(((ResourceCallbackAndExecutor) obj).f92cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f92cb.hashCode();
        }
    }
