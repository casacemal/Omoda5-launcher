...
class FragmentStateManager {
    ...
    void setFragmentManagerState(int r1) {
        this.mFragmentManagerState = r1;
    }

    int computeMaxState() {
        int r0 = this.mFragmentManagerState;
        if (this.mFragment.mFromLayout) {
            if (this.mFragment.mInLayout) {
                r0 = Math.max(this.mFragmentManagerState, 1);
            } else {
                r0 = Math.min(r0, 1);
            }
        }
        ...
        int r6 = C02831.$SwitchMap$androidx$lifecycle$Lifecycle$State[this.mFragment.mMaxState.ordinal()];
        if (r6 == 1) {
            return r0;
        }
        if (r6 == 2) {
            return Math.min(r0, 3);
        }
        if (r6 == 3) {
            return Math.min(r0, 1);
        }
        return Math.min(r0, -1);
    }
    ...
    void createView(FragmentContainer fragmentContainer) {
        ...
        if (this.mFragment.mView != null) {
            boolean z = false;
            this.mFragment.mView.setSaveFromParentEnabled(false);
            this.mFragment.mView.setTag(C0262R.id.fragment_container_view_tag, this.mFragment);
            if (viewGroup != null) {
