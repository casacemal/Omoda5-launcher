...
class OpReorderer {
    ...
    interface Callback {
        AdapterHelper.UpdateOp obtainUpdateOp(int r1, int r2, int r3, Object obj);
    ...
    private void swapMoveOp(List<AdapterHelper.UpdateOp> list, int r9, int r10) {
        AdapterHelper.UpdateOp updateOp = list.get(r9);
        AdapterHelper.UpdateOp updateOp2 = list.get(r10);
        int r0 = updateOp2.cmd;
        if (r0 == 1) {
            swapMoveAdd(list, r9, updateOp, r10, updateOp2);
        } else if (r0 == 2) {
            swapMoveRemove(list, r9, updateOp, r10, updateOp2);
        } else {
            if (r0 != 4) {
                return;
            }
            swapMoveUpdate(list, r9, updateOp, r10, updateOp2);
        }
    }

    void swapMoveRemove(List<AdapterHelper.UpdateOp> list, int r10, AdapterHelper.UpdateOp updateOp, int r12, AdapterHelper.UpdateOp updateOp2) {
        boolean z;
        boolean z2 = false;
        ...
        list.set(r10, updateOp2);
        if (updateOp.positionStart != updateOp.itemCount) {
            list.set(r12, updateOp);
        } else {
            list.remove(r12);
        }
        if (updateOpObtainUpdateOp != null) {
            list.add(r10, updateOpObtainUpdateOp);
        }
    }

    private void swapMoveAdd(List<AdapterHelper.UpdateOp> list, int r4, AdapterHelper.UpdateOp updateOp, int r6, AdapterHelper.UpdateOp updateOp2) {
        int r2 = updateOp.itemCount < updateOp2.positionStart ? -1 : 0;
        if (updateOp.positionStart < updateOp2.positionStart) {
            r2++;
        }
        ...
        updateOp2.positionStart += r2;
        list.set(r4, updateOp2);
        list.set(r6, updateOp);
    }
