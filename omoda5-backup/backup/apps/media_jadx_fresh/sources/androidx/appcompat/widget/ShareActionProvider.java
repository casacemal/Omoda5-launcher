...
public class ShareActionProvider extends ActionProvider {
    ...
    public View onCreateActionView() throws IllegalAccessException, NoSuchFieldException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        ...
        this.mContext.getTheme().resolveAttribute(C0105R.attr.actionModeShareDrawable, typedValue, true);
        ...
        activityChooserView.setDefaultActionButtonContentDescription(C0105R.string.abc_shareactionprovider_share_with_application);
        activityChooserView.setExpandActivityOverflowButtonContentDescription(C0105R.string.abc_shareactionprovider_share_with);
    ...
    public void onPrepareSubMenu(SubMenu subMenu) {
        ...
        int r3 = Math.min(activityCount, this.mMaxShownActivityCount);
        for (int r5 = 0; r5 < r3; r5++) {
            ResolveInfo activity = activityChooserModel.getActivity(r5);
            subMenu.add(0, r5, r5, activity.loadLabel(packageManager)).setIcon(activity.loadIcon(packageManager)).setOnMenuItemClickListener(this.mOnMenuItemClickListener);
        }
        if (r3 < activityCount) {
            SubMenu subMenuAddSubMenu = subMenu.addSubMenu(0, r3, r3, this.mContext.getString(C0105R.string.abc_activity_chooser_view_see_all));
            for (int r32 = 0; r32 < activityCount; r32++) {
                ResolveInfo activity2 = activityChooserModel.getActivity(r32);
                subMenuAddSubMenu.add(0, r32, r32, activity2.loadLabel(packageManager)).setIcon(activity2.loadIcon(packageManager)).setOnMenuItemClickListener(this.mOnMenuItemClickListener);
            }
        }
    }
