...
final class DecodeHelper<Transcode> {
    ...
    <R> void init(GlideContext glideContext, Object obj, Key key, int r4, int r5, DiskCacheStrategy diskCacheStrategy, Class<?> cls, Class<R> cls2, Priority priority, Options options, Map<Class<?>, Transformation<?>> map, boolean z, boolean z2, DecodeJob.DiskCacheProvider diskCacheProvider) {
    ...
    boolean isSourceKey(Key key) {
        ...
        for (int r2 = 0; r2 < size; r2++) {
            if (loadData.get(r2).sourceKey.equals(key)) {
                return true;
            }
        }
        return false;
    ...
    List<ModelLoader.LoadData<?>> getLoadData() {
        if (!this.isLoadDataSet) {
            ...
            for (int r1 = 0; r1 < size; r1++) {
                ModelLoader.LoadData<?> loadDataBuildLoadData = ((ModelLoader) modelLoaders.get(r1)).buildLoadData(this.model, this.width, this.height, this.options);
    ...
    List<Key> getCacheKeys() {
        if (!this.isCacheKeysSet) {
            ...
            for (int r3 = 0; r3 < size; r3++) {
                ModelLoader.LoadData<?> loadData2 = loadData.get(r3);
                if (!this.cacheKeys.contains(loadData2.sourceKey)) {
                    this.cacheKeys.add(loadData2.sourceKey);
                }
                for (int r5 = 0; r5 < loadData2.alternateKeys.size(); r5++) {
                    if (!this.cacheKeys.contains(loadData2.alternateKeys.get(r5))) {
                        this.cacheKeys.add(loadData2.alternateKeys.get(r5));
                    }
                }
            }
        }
