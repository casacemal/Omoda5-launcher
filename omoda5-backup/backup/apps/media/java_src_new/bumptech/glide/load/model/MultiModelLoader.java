...
class MultiModelLoader<Model, Data> implements ModelLoader<Model, Data> {
    ...
    public ModelLoader.LoadData<Data> buildLoadData(Model model, int r9, int r10, Options options) {
        ModelLoader.LoadData<Data> loadDataBuildLoadData;
        int size = this.modelLoaders.size();
        ArrayList arrayList = new ArrayList(size);
        Key key = null;
        for (int r3 = 0; r3 < size; r3++) {
            ModelLoader<Model, Data> modelLoader = this.modelLoaders.get(r3);
            if (modelLoader.handles(model) && (loadDataBuildLoadData = modelLoader.buildLoadData(model, r9, r10, options)) != null) {
