...
public abstract class FabTransformationBehavior extends ExpandableTransformationBehavior {
    ...
    private void createExpansionAnimation(View view, View view2, boolean z, boolean z2, FabTransformationSpec fabTransformationSpec, float f, float f2, List<Animator> list, List<Animator.AnimatorListener> list2) {
        ...
        if (view2 instanceof CircularRevealWidget) {
            ...
            if (z) {
                ...
                int r7 = (int) fCalculateRevealCenterX;
                int r11 = (int) fCalculateRevealCenterY;
                createPreFillRadialExpansion(view2, timing.getDelay(), r7, r11, f3, list);
                createPostFillRadialExpansion(view2, timing.getDelay(), timing.getDuration(), fabTransformationSpec.timings.getTotalDuration(), r7, r11, fWidth, list);
    ...
    private void createColorAnimation(View view, View view2, boolean z, boolean z2, FabTransformationSpec fabTransformationSpec, List<Animator> list, List<Animator.AnimatorListener> list2) {
        ...
        if (view2 instanceof CircularRevealWidget) {
            ...
            int r2 = 16777215 & backgroundTint;
            if (z) {
                if (!z2) {
                    circularRevealWidget.setCircularRevealScrimColor(backgroundTint);
                }
                objectAnimatorOfInt = ObjectAnimator.ofInt(circularRevealWidget, CircularRevealWidget.CircularRevealScrimColorProperty.CIRCULAR_REVEAL_SCRIM_COLOR, r2);
            } else {
    ...
    private float calculateTranslationY(View view, View view2, Positioning positioning) {
        ...
        calculateDependencyWindowBounds(view, rectF);
        calculateWindowBounds(view2, rectF2);
        int r2 = positioning.gravity & 112;
        if (r2 == 16) {
    ...
    private ViewGroup calculateChildContentContainer(View view) {
        View viewFindViewById = view.findViewById(C1199R.id.mtrl_child_content_container);
    ...
    private void createPreFillRadialExpansion(View view, long j, int r6, int r7, float f, List<Animator> list) {
        if (Build.VERSION.SDK_INT < 21 || j <= 0) {
            return;
        }
        Animator animatorCreateCircularReveal = ViewAnimationUtils.createCircularReveal(view, r6, r7, f, f);
        animatorCreateCircularReveal.setStartDelay(0L);
        animatorCreateCircularReveal.setDuration(j);
        list.add(animatorCreateCircularReveal);
    }

    private void createPostFillRadialExpansion(View view, long j, long j2, long j3, int r9, int r10, float f, List<Animator> list) {
        if (Build.VERSION.SDK_INT >= 21) {
            long j4 = j + j2;
            if (j4 < j3) {
                Animator animatorCreateCircularReveal = ViewAnimationUtils.createCircularReveal(view, r9, r10, f, f);
