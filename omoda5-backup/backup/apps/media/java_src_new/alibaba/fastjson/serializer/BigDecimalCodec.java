...
public class BigDecimalCodec implements ObjectSerializer, ObjectDeserializer {
    ...
    public void write(JSONSerializer jSONSerializer, Object obj, Object obj2, Type type, int r6) throws IOException {
        ...
        int r2 = bigDecimal.scale();
        if (SerializerFeature.isEnabled(r6, serializeWriter.features, SerializerFeature.WriteBigDecimalAsPlain) && r2 >= -100 && r2 < 100) {
            string = bigDecimal.toPlainString();
        } else {
            string = bigDecimal.toString();
        }
        if (r2 == 0 && string.length() >= 16 && SerializerFeature.isEnabled(r6, serializeWriter.features, SerializerFeature.BrowserCompatible) && (bigDecimal.compareTo(LOW) < 0 || bigDecimal.compareTo(HIGH) > 0)) {
