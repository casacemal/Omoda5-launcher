...
public class ASMUtils {
    ...
    public static String desc(Class<?> cls) {
        ...
        return ParamsPosition.f236L + type(cls) + ";";
    }
    ...
    public static String getPrimitiveLetter(Class<?> cls) {
        ...
        if (Byte.TYPE == cls) {
            return ParamsPosition.f234B;
        }
        if (Short.TYPE == cls) {
            return ExifInterface.LATITUDE_SOUTH;
        }
        if (Float.TYPE == cls) {
            return ParamsPosition.f235F;
        }
        if (Long.TYPE == cls) {
            return "J";
        }
    ...
    public static String[] lookupParameterNames(AccessibleObject accessibleObject) throws IOException {
        ...
        try {
            ...
            for (int r2 = 0; r2 < parameterNamesForMethod.length; r2++) {
                Annotation[] annotationArr = parameterAnnotations[r2];
                if (annotationArr != null) {
                    for (int r5 = 0; r5 < annotationArr.length; r5++) {
                        if ((annotationArr[r5] instanceof JSONField) && (strName = ((JSONField) annotationArr[r5]).name()) != null && strName.length() > 0) {
                            parameterNamesForMethod[r2] = strName;
                        }
                    }
                }
            }
