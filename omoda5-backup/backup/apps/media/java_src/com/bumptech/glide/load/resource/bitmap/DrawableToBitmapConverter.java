...
final class DrawableToBitmapConverter {
    ...
    static Resource<Bitmap> convert(BitmapPool bitmapPool, Drawable drawable, int r4, int r5) {
        ...
        if (current instanceof BitmapDrawable) {
            ...
            bitmapDrawToBitmap = drawToBitmap(bitmapPool, current, r4, r5);
            z = true;
        }
        if (!z) {
    ...
    private static Bitmap drawToBitmap(BitmapPool bitmapPool, Drawable drawable, int r8, int r9) {
        ...
        if (r9 == Integer.MIN_VALUE && drawable.getIntrinsicHeight() <= 0) {
            if (Log.isLoggable(TAG, 5)) {
