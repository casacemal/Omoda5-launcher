...
public class VideoDecoder<T> implements ResourceDecoder<T, Bitmap> {
    ...
    @Override // com.bumptech.glide.load.ResourceDecoder
    public Resource<Bitmap> decode(T t, int r10, int r11, Options options) throws IOException {
        ...
        try {
            try {
                ...
                Bitmap bitmapDecodeFrame = decodeFrame(mediaMetadataRetrieverBuild, jLongValue, num.intValue(), r10, r11, downsampleStrategy2);
    ...
    private static Bitmap decodeFrame(MediaMetadataRetriever mediaMetadataRetriever, long j, int r5, int r6, int r7, DownsampleStrategy downsampleStrategy) {
        Bitmap bitmapDecodeScaledFrame = (Build.VERSION.SDK_INT < 27 || r6 == Integer.MIN_VALUE || r7 == Integer.MIN_VALUE || downsampleStrategy == DownsampleStrategy.NONE) ? null : decodeScaledFrame(mediaMetadataRetriever, j, r5, r6, r7, downsampleStrategy);
        return bitmapDecodeScaledFrame == null ? decodeOriginalFrame(mediaMetadataRetriever, j, r5) : bitmapDecodeScaledFrame;
    }

    private static Bitmap decodeScaledFrame(MediaMetadataRetriever mediaMetadataRetriever, long j, int r12, int r13, int r14, DownsampleStrategy downsampleStrategy) {
        try {
            int r0 = Integer.parseInt(mediaMetadataRetriever.extractMetadata(18));
            int r1 = Integer.parseInt(mediaMetadataRetriever.extractMetadata(19));
            int r2 = Integer.parseInt(mediaMetadataRetriever.extractMetadata(24));
            if (r2 == 90 || r2 == 270) {
                r1 = r0;
                r0 = r1;
            }
            float scaleFactor = downsampleStrategy.getScaleFactor(r0, r1, r13, r14);
            return mediaMetadataRetriever.getScaledFrameAtTime(j, r12, Math.round(r0 * scaleFactor), Math.round(scaleFactor * r1));
        } catch (Throwable th) {
    ...
    private static Bitmap decodeOriginalFrame(MediaMetadataRetriever mediaMetadataRetriever, long j, int r3) {
        return mediaMetadataRetriever.getFrameAtTime(j, r3);
    }
    ...
    static final class ByteBufferInitializer implements MediaMetadataRetrieverInitializer<ByteBuffer> {
        ...
        public void initialize(MediaMetadataRetriever mediaMetadataRetriever, final ByteBuffer byteBuffer) throws IllegalArgumentException {
            mediaMetadataRetriever.setDataSource(new MediaDataSource() { // from class: com.bumptech.glide.load.resource.bitmap.VideoDecoder.ByteBufferInitializer.1
                ...
                public int readAt(long j, byte[] bArr, int r6, int r7) {
                    if (j >= byteBuffer.limit()) {
                        return -1;
                    }
                    byteBuffer.position((int) j);
                    int r3 = Math.min(r7, byteBuffer.remaining());
                    byteBuffer.get(bArr, r6, r3);
                    return r3;
                }
