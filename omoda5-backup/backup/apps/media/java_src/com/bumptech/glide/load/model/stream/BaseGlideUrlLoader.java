...
public abstract class BaseGlideUrlLoader<Model> implements ModelLoader<Model, InputStream> {
    ...
    public ModelLoader.LoadData<InputStream> buildLoadData(Model model, int r5, int r6, Options options) {
        ModelCache<Model, GlideUrl> modelCache = this.modelCache;
        GlideUrl glideUrl = modelCache != null ? modelCache.get(model, r5, r6) : null;
        if (glideUrl == null) {
            String url = getUrl(model, r5, r6, options);
            if (TextUtils.isEmpty(url)) {
                return null;
            }
            GlideUrl glideUrl2 = new GlideUrl(url, getHeaders(model, r5, r6, options));
            ModelCache<Model, GlideUrl> modelCache2 = this.modelCache;
            if (modelCache2 != null) {
                modelCache2.put(model, r5, r6, glideUrl2);
            }
            glideUrl = glideUrl2;
        }
        List<String> alternateUrls = getAlternateUrls(model, r5, r6, options);
        ModelLoader.LoadData<InputStream> loadDataBuildLoadData = this.concreteLoader.buildLoadData(glideUrl, r5, r6, options);
    ...
    protected List<String> getAlternateUrls(Model model, int r2, int r3, Options options) {
        return Collections.emptyList();
    }

    protected Headers getHeaders(Model model, int r2, int r3, Options options) {
