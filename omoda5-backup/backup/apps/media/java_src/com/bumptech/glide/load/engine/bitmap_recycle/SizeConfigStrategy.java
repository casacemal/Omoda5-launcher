...
public class SizeConfigStrategy implements LruPoolStrategy {
    ...
    @Override // com.bumptech.glide.load.engine.bitmap_recycle.LruPoolStrategy
    public Bitmap get(int r3, int r4, Bitmap.Config config) {
        Key keyFindBestKey = findBestKey(Util.getBitmapByteSize(r3, r4, config), config);
        Bitmap bitmap = this.groupedMap.get(keyFindBestKey);
        if (bitmap != null) {
            decrementBitmapOfSize(Integer.valueOf(keyFindBestKey.size), bitmap);
            bitmap.reconfigure(r3, r4, config);
        }
        return bitmap;
    }

    private Key findBestKey(int r9, Bitmap.Config config) {
        Key key = this.keyPool.get(r9, config);
        for (Bitmap.Config config2 : getInConfigs(config)) {
            Integer numCeilingKey = getSizesForConfig(config2).ceilingKey(Integer.valueOf(r9));
            if (numCeilingKey != null && numCeilingKey.intValue() <= r9 * 8) {
                if (numCeilingKey.intValue() == r9) {
                    if (config2 == null) {
                        if (config == null) {
                            return key;
                        }
    ...
    public String logBitmap(int r1, int r2, Bitmap.Config config) {
        return getBitmapString(Util.getBitmapByteSize(r1, r2, config), config);
    }
    ...
    static class KeyPool extends BaseKeyPool<Key> {
        ...
        public Key get(int r1, Bitmap.Config config) {
            Key key = get();
            key.init(r1, config);
            return key;
        }
    ...
    static final class Key implements Poolable {
        ...
        Key(KeyPool keyPool, int r2, Bitmap.Config config) {
            this(keyPool);
            init(r2, config);
        }

        public void init(int r1, Bitmap.Config config) {
            this.size = r1;
            this.config = config;
        }
    ...
    static String getBitmapString(int r2, Bitmap.Config config) {
        return "[" + r2 + "](" + config + ")";
    }
    ...
    private static Bitmap.Config[] getInConfigs(Bitmap.Config config) {
        ...
        int r0 = C05451.$SwitchMap$android$graphics$Bitmap$Config[config.ordinal()];
        if (r0 == 1) {
        ...
        if (r0 != 3) {
            return r0 != 4 ? new Bitmap.Config[]{config} : ALPHA_8_IN_CONFIGS;
        }
    ...
    static /* synthetic */ class C05451 {
        ...
        static {
            ...
            try {
                r0[Bitmap.Config.ARGB_8888.ordinal()] = 1;
