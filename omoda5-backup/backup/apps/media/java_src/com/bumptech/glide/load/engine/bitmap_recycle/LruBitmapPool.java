...
public class LruBitmapPool implements BitmapPool {
    ...
    public Bitmap get(int r1, int r2, Bitmap.Config config) {
        Bitmap dirtyOrNull = getDirtyOrNull(r1, r2, config);
        if (dirtyOrNull != null) {
            dirtyOrNull.eraseColor(0);
            return dirtyOrNull;
        }
        return createBitmap(r1, r2, config);
    }

    @Override // com.bumptech.glide.load.engine.bitmap_recycle.BitmapPool
    public Bitmap getDirty(int r1, int r2, Bitmap.Config config) {
        Bitmap dirtyOrNull = getDirtyOrNull(r1, r2, config);
        return dirtyOrNull == null ? createBitmap(r1, r2, config) : dirtyOrNull;
    }

    private static Bitmap createBitmap(int r0, int r1, Bitmap.Config config) {
        if (config == null) {
            config = DEFAULT_CONFIG;
        }
        return Bitmap.createBitmap(r0, r1, config);
    }
    ...
    private synchronized Bitmap getDirtyOrNull(int r6, int r7, Bitmap.Config config) {
        Bitmap bitmap;
        assertNotHardwareConfig(config);
        bitmap = this.strategy.get(r6, r7, config != null ? config : DEFAULT_CONFIG);
        if (bitmap == null) {
            if (Log.isLoggable(TAG, 3)) {
                Log.d(TAG, "Missing bitmap=" + this.strategy.logBitmap(r6, r7, config));
            }
            this.misses++;
        } else {
            this.hits++;
        ...
        if (Log.isLoggable(TAG, 2)) {
            Log.v(TAG, "Get bitmap=" + this.strategy.logBitmap(r6, r7, config));
        }
        dump();
        return bitmap;
    }
    ...
    public void trimMemory(int r5) {
        if (Log.isLoggable(TAG, 3)) {
            Log.d(TAG, "trimMemory, level=" + r5);
        }
        if (r5 >= 40 || (Build.VERSION.SDK_INT >= 23 && r5 >= 20)) {
            clearMemory();
        } else if (r5 >= 20 || r5 == 15) {
