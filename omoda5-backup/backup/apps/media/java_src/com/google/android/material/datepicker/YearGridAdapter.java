...
class YearGridAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int r3) {
        return new ViewHolder((TextView) LayoutInflater.from(viewGroup.getContext()).inflate(C1199R.layout.mtrl_calendar_year, viewGroup, false));
    }

    @Override // androidx.recyclerview.widget.RecyclerView.Adapter
    public void onBindViewHolder(ViewHolder viewHolder, int r9) {
        int yearForPosition = getYearForPosition(r9);
        String string = viewHolder.textView.getContext().getString(C1199R.string.mtrl_picker_navigate_to_year_description);
    ...
    int getPositionForYear(int r1) {
        return r1 - this.materialCalendar.getCalendarConstraints().getStart().year;
    }

    int getYearForPosition(int r1) {
        return this.materialCalendar.getCalendarConstraints().getStart().year + r1;
    }
}
