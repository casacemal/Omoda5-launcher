...
0x21402006 -> mGearPosition         # GERÇEK Vites Pozisyonu (D1, D2 destekli)
0x2140203f -> mManualGearPosition   # Manuel Vites Pozisyonu

# mGearPosition (0x21402006) için Kesinleşmiş Gerçek Vites Tablosu:
# NOT: Standart 11400400 sensörü Chery'de bozuktur, sürekli P (4) gösterir. Vitesleri ekrandaki gibi (D1, D2) okumak için bu sensör şarttır.
| Araç Ekranı | Sensör Çıktısı (int32Values) |
|-------------|------------------------------|
| **P**       | 1                            |
| **R**       | 2                            |
| **N**       | 3                            |
...
## 4. VHAL İLETİŞİM PROTOKOLLERİ VE GÜVENLİK DUVARLARINI AŞMA (KRONOLOJİK ANALİZ)

Omoda 5 (Semidrive T19C) ünitesinde VHAL (Vehicle Hardware Abstraction Layer) verilerine (Örn: Hız, Vites) dışarıdan (3. parti) bir uygulama ile erişebilmek için yapılan tüm denemeler, alınan hatalar ve ulaşılan "Kesin Çözüm" aşağıda detaylandırılmıştır. Bu arşiv, aynı hataların tekrar yapılmaması için altın değerindedir.

### 4.1. DENEME 1: CarPropertyManager (OEM System Signature Engeli)
- **Yaklaşım:** Android'in standart `CarPropertyManager` kütüphanesi kullanılarak `getProperty()` çağrısı yapıldı.
- **Sonuç:** **BAŞARISIZ**.
- **Kök Neden:** Android güvenlik mimarisi gereği `android.car.permission.CAR_SPEED` gibi donanım seviyesi verilere sadece cihaz üreticisi tarafından (OEM System Signature) imzalanmış uygulamalar erişebilir. Dışarıdan `pm install` ile kurulan uygulamalar yetki reddi alır (SecurityException) ve sensörler okunamaz.

### 4.2. DENEME 2: Localhost 5555 Üzerinden AdbClient (Soket Çökmesi)
- **Yaklaşım:** Bilgisayardan `adb shell dumpsys car_service get-property-value...` komutu çalıştığı için, uygulamanın içine TCP üzerinden `127.0.0.1:5555` portuna bağlanan yerel bir ADB Client (`DemoAdbClient.kt`) inşa edildi. Komutların cihazın kendi loopback ağı üzerinden atılması hedeflendi.
- **Sonuç:** **BAŞARISIZ**.
- **Kök Neden:** Semidrive ağ politikaları ve kısıtlamaları yüzünden soketler `Connection Refused` veya `EOFException` (End of File) hatası verdi. Port açılsa bile veri okuma sırasında arka planda sessizce kilitlendi (Silent Hang). Kesintisiz ve stabil bir veri akışı sağlanamadı.

### 4.3. DENEME 3: Shizuku Entegrasyonu (Servis Başlatılamadı)
- **Yaklaşım:** Root yetkisi olmadan ayrıcalıklı API'leri kullanmaya yarayan Shizuku aracı denenerek `Shizuku.newProcess` üzerinden doğrudan shell komutu çalıştırılmak istendi.
- **Sonuç:** **BAŞARISIZ**.
- **Kök Neden:** Shizuku mimarisinin çalışabilmesi için arka planda Shizuku servisinin cihazda `bind` (bağlı) olması gerekir. Semidrive sisteminde bu servisi aktif edebilecek açık bir yol bulunamadığı için Shizuku entegrasyonu tamamen iptal edildi.

### 4.4. FİNAL VE KESİN ÇÖZÜM: DUMP İzni ve 0x Prefix Bug'ının Aşılması
- **Yaklaşım:** Bütün ağ ve soket katmanları devreden çıkarıldı. Komutun doğrudan uygulama içinden çalıştırılmasına (`Runtime.getRuntime().exec()`) karar verildi. 
- **İlk Hata (Permission Denied):** Android, `dumpsys` aracıyla sistem servislerini okumak için özel `android.permission.DUMP` izni arar. Bu izin uygulamada olmadığı için ilk denemede erişim reddedildi.
- **Çözüm (Yetki Enjeksiyonu):** `AndroidManifest.xml` içerisine `<uses-permission android:name="android.permission.DUMP" />` eklendi. Cihaz bilgisayara bağlıyken şu altın komut çalıştırılarak uygulamaya sistem root yetkilerine yakın bir log çekme yetkisi bahşedildi:
  ```bash
  adb shell pm grant com.omoda5.launcher android.permission.DUMP
  ```
- **İkinci Hata (CarPropertyService Çökmesi):** Uygulama `dumpsys car_service get-property-value 0x11600207 0` komutunu attığında, komut başarıyla cihaza iletildi ancak cihaz çöktü! Logcat incelendiğinde `CarPropertyService.java` içinde `java.lang.NumberFormatException` hatası alındığı görüldü.
- **Kök Neden:** Semidrive mühendislerinin yazdığı VHAL implementasyonu standart dışıydı. Hex ID'lerin başındaki `0x` önekini (prefix) silecek bir parse mekanizması yazmayı unutmuşlardı. Bu yüzden cihaz `0x11600207` stringini sayıya çeviremiyor ve çöküyordu.
- **Son Vuruş:** Property ID gönderilirken baştaki `0x` kodu çıkartıldı ve sadece `11600207` olarak gönderildi.

### 4.5. Nihai Kod Formülü (Çalışan Yapı)
Bu formül ile saniyede bir milisaniyelik hızla `dumpsys` üzerinden hiçbir ağ katmanına bulaşmadan araç verileri stabil şekilde okunabilmektedir.

```kotlin
// Speed okuma:
val proc = Runtime.getRuntime().exec(arrayOf("sh", "-c", "dumpsys car_service get-property-value 11600207 0"))
