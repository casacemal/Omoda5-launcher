...
object RelayClient {
    ...
    fun autoConnect() {
        val url = serverUrl ?: GlobalState.serverIp.value
        val code = pairingCode ?: "OMODA5"
        
        if (!url.isNullOrBlank() && !code.isNullOrBlank()) {
            Log.i(TAG, "Auto-connecting to $url with code $code")
            connect(url, code)
        }
    }
    ...
    private fun doConnect(serverUrl: String, pairingCode: String) {
        ...
        val request = Request.Builder()
            ...
            .addHeader("X-Hermes-Token", pairingCode)
            .addHeader("X-API-Key", GlobalState.HERMES_API_KEY)
            .addHeader("X-Hermes-Session-Key", GlobalState.sessionKey.value)
            .addHeader("User-Agent", "Hermes-Android-Bridge/1.0")
            .addHeader("Origin", "http://$serverUrl")
            .addHeader("Sec-WebSocket-Protocol", "hermes-bridge-v1")
            .build()
    ...
    private fun scheduleReconnect() {
        ...
        if (reconnectJob?.isActive == true) {
            Log.d(TAG, "Reconnection job is already active. Skipping duplicate scheduling.")
            return
        }
        ...
        reconnectJob = scope?.launch {
            ...
            while (shouldReconnect && !isConnected && retries < MAX_RETRIES) {
                ...
                if (shouldReconnect && !isConnected) {
                    doConnect(url, code)
                    var waitTime = 0
                    while (!isConnected && shouldReconnect && waitTime < 5000) {
                        delay(200)
                        waitTime += 200
                    }
                    if (!isConnected) {
    ...
    private suspend fun handleMessage(ws: WebSocket, text: String) {
        ...
        try {
            ...
            val fallbackId = try {
                JsonParser.parseString(text).asJsonObject.get("request_id")?.asString ?: ""
            } catch (_: Exception) { "" }
            try {
                val errorResponse = JsonObject().apply {
                    addProperty("request_id", fallbackId)
                    add("result", gson.toJsonTree(mapOf("error" to (e.message ?: "Unknown error"))))
